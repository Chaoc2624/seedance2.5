 ...
 */
export interface CheckoutSession<T = unknown> {
  provider: string;

  checkoutParams: T; // checkout request params
  checkoutInfo: CheckoutInfo; // checkout info after checkout success
  checkoutResult: T; // provider checkout result
...
export interface PaymentEvent<T = unknown> {
  ...
  eventResult: T; // provider event result

  paymentSession?: PaymentSession<T>;
}
