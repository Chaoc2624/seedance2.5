...
export class CreemProvider implements PaymentProvider {
  ...
  async getPaymentSession({
    ...
    return this.buildPaymentSessionFromCheckoutSession(session);
  }
  ...
  async getPaymentEvent({ req }: { req: Request }): Promise<PaymentEvent> {
    ...
    if (eventType === PaymentEventType.CHECKOUT_SUCCESS) {
      ...
      paymentSession = await this.buildPaymentSessionFromInvoice(event.object);
  ...
  async searchProducts(): Promise<CreemProduct[]> {
  ...
  async cancelSubscription({
    ...
    return this.buildPaymentSessionFromSubscription(result);
  }
  ...
  private async makeRequest(endpoint: string, method: string, data?: any) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `request creem api failed with status: ${response.status}${body ? `: ${body}` : ''}`
      );
    }
