...
export class PayPalProvider implements PaymentProvider {
  ...
  async createPayment({
    ...
    return this.createOneTimePayment(order);
  }
  ...
  async getPaymentSession({
    ...
    try {
      ...
      if (orderResult.status === 'APPROVED') {
        ...
        console.log('PayPal payment captured, new status:', orderResult.status);
      }
  ...
  async capturePayment(orderId: string): Promise<PaymentSession> {
    ...
    return this.buildPaymentSessionFromOrder(result);
  }
