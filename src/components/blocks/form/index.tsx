...
function buildFieldSchema(field: FormFieldType) {
  ...
  if (
    ...
    field.metadata?.max &&
    (field.metadata.max as number) > 1
  ) {
...
function buildDefaultValues(
  ...
  const normalizeSelectValue = (field: FormFieldType, value: unknown) => {
    const rawValue = value !== null && value !== undefined ? String(value) : '';
    const trimmedValue = rawValue.trim();
    const options = field.options ?? [];
    const exactOption = options.find((option) => option.value === trimmedValue);
  ...
  fields.forEach((field) => {
    ...
    if (field.type === 'switch') {
      ...
      defaultValues[field.name] =
        normalizeSelectValue(field, val) ??
        normalizeSelectValue(field, field.value) ??
        '';
    } else {
...
export function Form({
  ...
  const defaultValues = useMemo(
    () => buildDefaultValues(fields, data),
    [fields, data]
  );
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      ...
      if (res.redirect_url) {
        // dynamic redirect bypassing type checking
        const routerPush = router.push as (path: string) => void;
        routerPush(res.redirect_url);
      }
