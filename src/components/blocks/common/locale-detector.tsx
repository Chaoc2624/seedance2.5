...
export function LocaleDetector() {
  ...
  const detectBrowserLocale = (): string | null => {
    ...
    const normalizedLocale = normalizeLocale(browserLang);
  ...
  useEffect(() => {
    ...
    const preferredLocale = normalizeLocale(cacheGet(PREFERRED_LOCALE_KEY));
  ...
  return (
    <>
      <div
        ref={bannerRef}
        className="fixed top-0 right-0 left-0 z-[51] hidden bg-gradient-to-r from-primary to-primary/80 text-primary-foreground shadow-lg md:block"
      >
        <div className="container py-2.5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-1 items-center gap-3">
              <span className="text-sm">
                {isChineseLocale(browserLocale)
                  ? `偵測到瀏覽器語言為：${targetLocaleName}，是否切換？`
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              ...
              >
                {isChineseLocale(browserLocale)
                  ? `切换到${targetLocaleName}`
                  : 'Switch'}
              </Button>
              <button
                onClick={handleDismiss}
                className="flex-shrink-0 rounded bg-primary/10 p-1 transition-colors"
                aria-label="Close"
              >
                <X className="h-4 w-4" />
              </button>
