...
import { NewCredit } from './credit.server';
import {
  NewSubscription,
  UpdateSubscription,
...
export async function updateOrderInTransaction({
  ...
  const result = await db().transaction(async (tx: any) => {
    ...
    if (newSubscription) {
      let existingSubscription: typeof subscription.$inferSelect | null = null;
...
export async function updateSubscriptionInTransaction({
  ...
  const result = await db().transaction(async (tx: any) => {
    ...
    // deal with credit
    if (newCredit) {
      let existingCredit: typeof credit.$inferSelect | null = null;
