...
function createGetSessionThrottledFetch({
  ...
  return throttledFetch as typeof fetch;
}

// Client-side throttle to avoid get-session request storms in browser.
// Note: must be VITE_* to be inlined into client bundles.
const AUTH_GET_SESSION_MIN_INTERVAL_MS =
  Number(process.env.VITE_AUTH_GET_SESSION_MIN_INTERVAL_MS) || 2000;
